...
- Artifact verification: one command, proven in both directions, with a signature-control suite
  (`tests/unity/aab-signature-controls.test.sh`) and a verdict decision table
  (`tests/unity/artifact-verdict.test.py`) — the second exists because a known certificate
  mismatch was once reported as merely unconfirmed.

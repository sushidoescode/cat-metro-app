...
|---|---|---|---|---|---|---|---|
| P-0 | `cm_district_night_harbor` (already signed; restated) | non-consumable, 10 levels | **PASS** — remix of free mechanics; excluded from `LevelBand`, Daily pool, Cup | **PASS** — flat one-time price, no randomness, no currency | **PASS** — one-time, restorable; revocation retains stars/scores | **inapplicable** — content, not a cosmetic overlay; no readability change | **IN CATALOG (D-0)** |
...
| P-3 | `cm_district_signal_works` | non-consumable, 10 levels | **BLOCKED, not failed** — its `cooldown`/`gate` vocabulary is UNBUILT and not yet free anywhere; law 1 forbids paid-first mechanics | **PASS** if unblocked | **PASS** if unblocked | **inapplicable** | **CONDITIONAL — no SKU proposed (D-3)** |
...
Every PASS in the law-1 column additionally depends on the four district-level invariants restated per
district in `districts.md` §3: no `L9xx` id in `GameRoot.LevelBand`; exclusion from the Daily Line
rotating pool and Cup rounds; the CM-R10.6/A-27 faucet-parity inequality (Q-M2); no global-board channel
(ADR-0010 scopes boards to Daily/Cup).
...
| Paid-district rounds in the weekly District Cup | Laws 1, 6 | Cup stays free; no purchase may confer medal or rank eligibility | **DROPPED** |
| A new mechanic debuting in a paid district | Law 1 | "Every mechanic … remain free" — paid-first mechanics are forbidden | **DROPPED (see D-3, blocked not shipped)** |

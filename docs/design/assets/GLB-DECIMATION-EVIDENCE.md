...
Before the 2026-08-18 correction, the final custody-hardened refresh
rendered all 15 source/derivative pairs
...
byte-identical to both the r3 and the then-retained pre-curation derivatives.
Each fresh sidecar was identical after excluding only its expected fresh
`tool.timestamp_utc`, and a fresh `glb_metrics.py` inspection matched every
recorded output metric and derivative hash. The curated loaf and wave rows,
hashes, and renders above supersede those two historical pre-curation records;
the other 13 records remain unchanged.

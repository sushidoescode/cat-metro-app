...
namespace CatMetro.Presentation.Audio
{
    ...
    public sealed class GameplayAudioCueTracker
    {
        ...
        public void Rebaseline(int deliveries, OutcomeKind outcome)
        {
        ...
        public GameplayAudioCues Observe(int deliveries, OutcomeKind outcome)
        {
            if (!_hasBaseline)
            {
                Rebaseline(deliveries, outcome);
    ...
    public sealed class GameAudio : MonoBehaviour
    {
        ...
        public void BindSession(GameSession session)
        {
            ...
            if (session == null)
            {
                _cueTracker.Rebaseline(0, OutcomeKind.Running);
                return;
            }
            _cueTracker.Rebaseline(session.State.Deliveries, session.State.Outcome.Kind);
        }
        ...
        public void Observe(GameSession session, bool gameplayVisible)
        {
            ...
            GameplayAudioCues cues = _cueTracker.Observe(
                state.Deliveries, state.Outcome.Kind);
        ...
        public static bool HasMovingTrain(SimulationState state)
        {
            ...
            for (int i = 0; i < state.Trains.Length; i++)
                if (state.Trains[i].Id != 0 && state.Trains[i].State == TrainState.OnEdge)
                    return true;
            return false;
        }

        // The current Domain deliberately halts on a wrong-station arrival. The presenter reads
        // the partially advanced snapshot in GameRoot's catch path to distinguish that guarded
        // boundary from unrelated exceptions, without adding a simulation event or mutation.
        public static bool HasWrongStationArrival(GameSession session) =>
            session != null && HasWrongStationArrival(session.State);

        public static bool HasWrongStationArrival(SimulationState state)
        {
            if (state?.Graph == null || state.Trains == null) return false;
            var graph = state.Graph;
            if (graph.StationNode == null || graph.StationAccepts == null
                || graph.StationAccepts.Length < graph.StationNode.Length)
                return false;
            for (int t = 0; t < state.Trains.Length; t++)
            {
                var train = state.Trains[t];
                if (train.Id == 0 || train.State != TrainState.AtNode) continue;

                for (int station = 0; station < graph.StationNode.Length; station++)
                {
                    if (graph.StationNode[station] != train.NodeId) continue;
                    if (train.Color == CatColor.Wild) break;

                    bool accepted = false;
                    var accepts = graph.StationAccepts[station];
                    if (accepts == null) return false;
                    for (int color = 0; color < accepts.Length; color++)
                    {
                        if (accepts[color] != train.Color) continue;
                        accepted = true;
                        break;
                    }

...
if grep -Eq "$sm|$st" <<<"$leak"; then echo "  FAIL: sentinel key leaked in dry-run output"; fail=1; else echo "  ok: no key leak in dry-run"; fi
leak2=$(MESHY_API_KEY="$sm" TRIPO_API_KEY="$st" bash "$script" 2>&1 || true)
if grep -Eq "$sm|$st" <<<"$leak2"; then echo "  FAIL: sentinel key leaked in usage output"; fail=1; else echo "  ok: no key leak in usage"; fi
...
if grep -Eq "$sm" <<<"$leak3"; then echo "  FAIL: sentinel key leaked on the live meshy path"; fail=1; else echo "  ok: no key leak on live meshy path"; fi
...
if grep -Eq "$st" <<<"$leak4"; then echo "  FAIL: sentinel key leaked on the live tripo path"; fail=1; else echo "  ok: no key leak on live tripo path"; fi

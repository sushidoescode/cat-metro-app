#!/usr/bin/env bash
# Artifact gate for the shipped cooldown/alternation quartet. It reads authored values and
# replay evidence instead of preserving pre-ladder golden numbers.
set -uo pipefail
cd "$(git rev-parse --show-toplevel)"
fail() { echo "alternation-band.test.sh: FAIL — $1"; exit 1; }
tmp=$(mktemp -d "${TMPDIR:-/tmp}/cm-alternation-XXXXXX") || fail "mktemp failed"
trap 'rm -rf "$tmp"' EXIT

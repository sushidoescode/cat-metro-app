...
| **Q-K** | **`TimeOut` camera target** — largest queue at fail tick, ties → lowest node id. **RATIFIED (ratified by the human in-session 2026-08-05; recorded by the phases-6-10 agent)** | CM-C3 | Shipped as ratified in CM-C3 (#22). |
...
Each contract owns a **disjoint set of file paths**. No two contracts write the same file, with two
enumerated exception classes (registration files, and the staged derived tree — both below). Path ownership is the review test: a diff that
...
| **CM-C2b** | `unity/Assets/Scripts/Presentation/Board/**`, `/Input/**`, `/Diagnostics/**` · `unity/Assets/Scripts/Presentation/Hud/**` **EXCEPT `.../Hud/WavePreview/**`** · `unity/Assets/Scripts/Application/**` **EXCEPT `Application/Retry/**`, `Application/Save/**`, `Application/Analytics/**`, `Application/EventTaxonomy/**`** *(EventTaxonomy carved out to CM-C9 — ratified human in-session 2026-08-05, delegated batch)* · `unity/Assets/Scripts/Bootstrap/**` · `unity/Assets/Scenes/Game*` · `unity/Assets/StreamingAssets/content/**` · `unity/Assets/StreamingAssets/config/**` *(granted by the 2026-08-04 recut for criterion 10 — closes Q-Y; **derived tree** from CM-C10 onward — commit rights are defined by the staged-derived-tree exception class below, never by hand-written bytes — ratified human in-session 2026-08-05, N1)* · `unity/Assets/Resources/Strings/ui.csv` (created here) · `unity/Assets/Tests/EditMode/Engine/**` **EXCEPT `unity/Assets/Tests/EditMode/Engine/ContentSync/**`** *(reserved for the assert-only ContentSync re-cut — ratified human in-session 2026-08-05, delegated batch)* · `unity/Assets/Tests/PlayMode/Board/**` · `tests/unity/**` **EXCEPT `tests/unity/failure.test.sh`** |
...
| **CM-C5** *(rows inherited by **CM-C5.1**, the dead-`newMechanic` gate — ratified human in-session 2026-08-05, delegated batch)* | `unity/Assets/Scripts/Content/Validation/**` · **`dotnet/CatMetro.Validator/**`** (the console host: file I/O + the on-disk `IContentSource`) · `unity/Assets/Tests/EditMode/Pure/Validation/**` · `tests/validation/**` (**incl. the new leaf `tests/validation/fixtures/dead-mechanic/**`**) · `scripts/validate-content.sh` · `config/validator_thresholds.json` |
| **CM-C6** | `unity/Assets/Scripts/Content/Daily/**` · **`dotnet/CatMetro.DailyTools/**`** (the console host: file I/O + artifact writing) · `unity/Assets/Tests/EditMode/Pure/Content/Daily/**` *(relocated by #73 per the Wave-2 addendum; longest-prefix out-resolves CM-C2a’s `Pure/Content/**`)* · `tests/daily/**` · `scripts/validate-dailies.sh` · `config/daily_pipeline.json` |
...
| **CM-C9** *(tranche 3 — event taxonomy; ratified human in-session 2026-08-05, delegated batch)* | `unity/Assets/Scripts/Application/EventTaxonomy/**` *(carved out of CM-C2b's `Application/**`, line 117)* · `unity/Assets/Tests/EditMode/Pure/EventTaxonomy/**` · `tests/taxonomy/**` · `tests/fixtures/taxonomy-bad/**` |
| **CM-C10** *(tranche 3 — content stager; ratified human in-session 2026-08-05, N1)* | `scripts/stage-content.sh` · `tests/staging/**` · `tests/fixtures/staging/**` *(staged-tree commits ride the staged-derived-tree exception class below; the `scripts/build.sh` stage-hook append rides the registration class)* |
| **CM-C11** *(tranche 3 — L006–L010 alternation band; ratified human in-session 2026-08-05, delegated batch)* | `content/levels/L006.json`, `content/levels/L007.json`, `content/levels/L008.json`, `content/levels/L009.json`, `content/levels/L010.json` *(new files only, inside CM-C2a's `content/levels/**`, line 116 — the L002-L005 (#15) precedent; enumerated explicitly so the resolution rule at :108 stays checker-implementable — review L-3)* · `unity/Assets/Tests/EditMode/Pure/Corpus/**` **(unowned before this amendment)** · `tests/corpus/**` **(unowned before this amendment)** |
| **UX lane** *(`state/handoffs/SESSION-HANDOFF-ux.md:28-30`; claim narrowed here — ratified human in-session 2026-08-05, delegated batch)* | `unity/Assets/Scripts/Presentation/**` per its own contracts · **append-only rows** in `unity/Assets/Resources/Strings/ui.csv` · `tests/unity/editmode.test.sh` **only inside the explicit gate-evolution contract, after CM-C3-DEVCAP merges** (`SESSION-HANDOFF-ux.md:36-39`) · **the test folders its own contracts create** under `unity/Assets/Tests/**`. **The handoff's prose claim to "your own tests under `unity/Assets/Tests/**`" is NOT a claim to the whole tree**: it resolves under the ordered-longest-prefix rule (`:108-111`) and expressly excludes every folder owned by a row above — in particular `Pure/Domain/**` (CM-C1), `Pure/Content/**` (CM-C2a), `EditMode/Engine/**` + `PlayMode/Board/**` (CM-C2b), `PlayMode/Failure/**` + `Pure/Retry/**` (CM-C3), `Pure/Solver/**` (CM-C4), `Pure/Validation/**` (CM-C5/CM-C5.1), `Pure/Content/Daily/**` (CM-C6, relocated by #73), `Pure/Save/**` (CM-C7), `Pure/Analytics/**` (CM-C8), `Pure/EventTaxonomy/**` (CM-C9), `EditMode/Engine/ContentSync/**` (reserved, A2) and `Pure/Corpus/**` (CM-C11). |
...
**new rows** in `unity/Assets/Resources/Strings/ui.csv`, and — **new entries in the `references`
array only** — in `unity/Assets/Tests/EditMode/CatMetro.Tests.EditMode.asmdef` *(ratified human
in-session 2026-08-05, delegated batch)*: **never** the `name`, `includePlatforms`,
`defineConstraints`, `autoReferenced`, `overrideReferences` or `noEngineReferences` fields, never a
reordering or removal of an existing entry, and never a second `.asmdef` under
`unity/Assets/Tests/EditMode/`. A contract using this exception **names the appended reference in its
PR description**, because an asmdef edit changes what compiles in the Unity host for every lane.
Also in this class *(N1 repair, ratified human in-session 2026-08-05)*: **append-only**
stage-invocation additions to `scripts/build.sh` — an unowned stub today; the engine-build
contract that replaces the stub will claim the file with its own ownership row.

**Staged derived tree (the second exception class — ratified human in-session 2026-08-05, N1).**
`unity/Assets/StreamingAssets/content/**` and `unity/Assets/StreamingAssets/config/**` (CM-C2b's
...
(so `Pure/Content/**`, `Pure/Solver/**`, `Pure/Validation/**`, `Pure/Content/Daily/**`, `Pure/Save/**`,

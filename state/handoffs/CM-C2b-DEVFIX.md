...
- 2026-08-05 review round 1 (PR #30): NOT-mergeable — F-1 intermediate-texture pin shipped as the URP default Always (the exact implicit-default failure criterion 2 exists to prevent; the byte-true asset paste is what made it findable) + F-2 ring-not-walked + F-3 vacuous vsync assert + F-4/F-5/F-6/F-7. Fixed: renderer asset byte -> Auto(0) + wrapper grep + 2 more typed asserts (main light PerPixel, LDR) + 2 more YAML greps (Forward, renderer index); ring asserted explicitly by name (unparented by design); vsync de-vacuumed (poke 1 -> assert policy drives 0, runtime-only); RED STUB residue removed; GreyboxMaterial comment made honest (F-7). CORRECTION to this log at :277: the intermediate-texture claim was false until this commit. F-6.3 (editor Play-Mode label screenshot) routed to the human: produce it, or accept the device screencap at re-measure as superseding label evidence. Additional-lights per-object limit 4 is inert while mode=Disabled — contract table reconciliation noted, asset left editor-generated.

- 2026-08-05 round 2 (PR #30): MERGEABLE with F-6.3 as the named human gate (reviewer states both readings; recommendation: human dispositions the label-screenshot leg before merge). Mutation re-probes now RED on all four (vsync-deletion, ring-unbind, intermediate-texture flip, rendering-mode flip). Nit fixes: N-1 stale-ring clear before the by-name assertion; N-2 single-renderer-entry count gate; N-3 PR table refreshed; N-4 noted — the :232 "every pinned setting" claim is scoped by this entry to the gated set (renderer count now gated; per-object light limit inert-while-Disabled, table reconciled). Merge WAITS on the human F-6.3 call.

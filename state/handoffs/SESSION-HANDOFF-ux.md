...
- 2026-08-06 — CM-UX-02 executed: DEVFIX gate honored (red only after #30 on main; post-DEVFIX baseline EM 353/353 + PM 38/38, CM-UX-01 pins re-verified); TMP essentials imported (45 assets, Packages untouched, shim disclosed+deleted); red 02c0e2e (22 right-reason) → green ace6c09 (EM 372/372, PM 46) → #33 visual-verification rule discharged with committed rendered frames (CTA + veil over the URP baseline, session-eyeballed). PR #35; UiChrome.mat scope note flagged for the reviewer. Review round 1: FIX-THEN-MERGE (2H+6M+10L) — the reviewer MEASURED the capture PNGs byte-wise and proved the CTA frame non-probative (RT/Screen mismatch); fixes applied incl. joining the geometry table to the shipped rect (H1), whole-root whitelist walk + Selectable/raycaster detection (H2), Screen-matched re-capture (M1), Won-row control (M2), per-row dpi + P9P rows (M4/L5), UiChrome pairing test (M6).
...
- 2026-08-06 — CM-UX-05 executed (parallel subagent, own worktree): frozen contract at anchor ca13801 → red 6b31971 (22 right-reason over EM 375 / PM 52 base) → green eb6695d (EM 393/393, PM 60/60) → #33 visual rule discharged (3 Screen-matched frames in evals/results/ux/cm-ux-05/, session-eyeballed: chip above CTA band, hidden-by-default control, persists-into-play). One declared existing-test edit: UiCsvDisciplineTests bound 7→8 per its own R1-L6 evolution path. Chip/station overlap on greybox framing flagged for the placement eyeball.
